...
| `ENGRAM_CUES` | `off` | no | `on` enables local regex-triggered input suggestions and opt-in learning. Changing it requires a restart. Similar reply wording is compared only in process memory; proposals and approved cues may retain redacted prompt text in `~/.engram/cues.json`. |
...
- **Local cues:** With `ENGRAM_CUES=on`, an approved Go regular expression may
  ...
  process-local anchor frame the user saw. A bounded local token/trigram score
...
- **Local state and logs:** `ENGRAM_HOME` contains `state.json`, `audit.jsonl`,
  ...
  explicitly approved cues. Fuzzy-comparison material is not persisted and is
  rebuilt only from replies observed after restart. Configured secrets are
  redacted before cue observation, but best-effort redaction cannot make
  arbitrary prompts safe.
