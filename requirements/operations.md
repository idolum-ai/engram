...
- `/status` shows version, uptime, session count, anchor mode, snapshot renderer
  ...
  its selected provider and model. It separately reports whether OpenAI voice
  transcription is configured and its admitted model.
...
- `engram preflight`, `engram status`, and `engram dry-start` validate the local
  ...
  polling. They report voice transcription separately from the selected guide.
