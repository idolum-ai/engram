...
- `engram signal <message>` writes `BEL`, establishes a new physical row with
  explicit CRLF, and writes one UTF-8 line beginning with the versioned literal
  prefix `[engram:upstream:v1] ` to its controlling terminal. The distinct
  marker prevents valid legacy payload text from being reinterpreted as
  version framing.
...
- Observation admits zero through eight leading ASCII spaces before either the
  exact versioned prefix or the legacy `[engram:upstream] ` prefix so terminal
  hosts such as Codex may present command output as an indented block. Only a valid versioned record whose payload is shorter than
  ...
  and reference evidence. Continuation rows are removed only after the complete
  record validates; malformed or truncated framing cannot consume ordinary
  pane output. Column-zero and legacy records remain one line.
