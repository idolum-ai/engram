...
func TestMarkdownExportsRegistryByCategory(t *testing.T) {
	t.Parallel()
	...
	for _, want := range []string{
		...
		"`//clear` sends the literal text `/clear`",
	} {
	...
	for _, meta := range All() {
		if !strings.Contains(text, "`"+markdownCell(meta.Usage)+"`") {
			t.Fatalf("Markdown() missing command %q", meta.Command)
		}
	}
}
