...
func (a *App) deliverUpstreamSignalLocked(ctx context.Context, observed state.TerminalSession, record upstream.Record, artifacts []string) {
	...
	if !ok || latest.Collapsed || latest.State != state.TerminalRunning || !latest.WatchEnabled || latest.AnchorChatID == 0 || latest.AnchorMessageID == 0 || latest.RetiringAnchorMessageID != 0 || !sameTerminalBinding(latest, observed) {
	...
	_, _, stateErr := a.Store.UpdateSession(latest.ID, func(session *state.TerminalSession) {
		if !session.Collapsed && session.AnchorMessageID == latest.AnchorMessageID && sameTerminalBinding(*session, latest) && session.State == state.TerminalRunning {
